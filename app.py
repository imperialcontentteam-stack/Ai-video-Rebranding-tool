...
# treatment for every brand while the selected brand still controls the logo,
# outro and content-page logo replacement.
INTRO_TITLE_COLOR = (255, 255, 255)
INTRO_PURPLE = (103, 57, 183)
INTRO_PILL_BORDER = (220, 190, 246)
TITLE_SHADOW_OFFSET = (4, 8)
TITLE_SHADOW_BLUR = 8
TITLE_SHADOW_ALPHA = 115
PILL_SHADOW_OFFSET = (0, 12)
PILL_SHADOW_BLUR = 12
PILL_SHADOW_ALPHA = 105
PILL_HEIGHT = 104
PILL_BORDER_WIDTH = 8
PILL_HORIZONTAL_PADDING = 170

# The generic Intro.mp4 contains the Aspirex logo. The logo area is removed
# dynamically with FFmpeg's delogo filter and the selected brand logo is then
# composited in its place, so GEL exports no longer retain the Aspirex mark.
INTRO_LOGO_DELOGO = (805, 875, 310, 115)  # x, y, width, height
INTRO_LOGO_MAX_W = 255
INTRO_LOGO_MAX_H = 82
INTRO_LOGO_CENTER_Y = 930
INTRO_LOGO_FADE_START = 0.12
INTRO_LOGO_FADE_DUR = 0.62
...
BRANDS = {
    "Aspirex": {
        ...
        "pill_bg":        (255, 255, 255),
        "pill_text":      INTRO_PURPLE,
        "pill_border":    INTRO_PILL_BORDER,
    },
    "GEL": {
        "prefix":         "gel",
        "logo":           "gel_logo.png",
...
def _title_line_gap(font_size: int, line_count: int) -> int:
    """Return a compact line gap close to the source intro typography."""
    if line_count <= 1:
        return 0
    # Two-line titles in the original intro have a wider visual gap; longer
    # titles need a tighter gap to remain inside the safe area.
    factor = 0.29 if line_count == 2 else 0.18
    return max(10, int(round(font_size * factor)))


def _visual_text_metrics(draw, lines: list[str], font) -> tuple[list[tuple[int, int, int, int]], int, int]:
    boxes = [draw.textbbox((0, 0), line or " ", font=font) for line in lines]
    size = int(getattr(font, "size", 48))
    gap = _title_line_gap(size, len(lines))
    total_h = sum(max(1, b[3] - b[1]) for b in boxes) + gap * max(0, len(lines) - 1)
    return boxes, gap, total_h
...
def fit_title(draw, text: str, max_w: int):
    ...
    for size in range(80, 23, -2):
        ...
        _, _, block_h = _visual_text_metrics(draw, lines, font)
...
def build_clean_patch_layers(intro_path: Path) -> Tuple[Image.Image, Image.Image]:
    ...
    # During the entrance animation the baked-in title travels beyond its final
    # glyph positions. A softly feathered rectangular safety mask prevents the
    # old wording from briefly appearing behind the replacement wording.
    title_motion_outer = np.zeros((TARGET_H, TARGET_W), dtype=np.uint8)
    cv2.rectangle(title_motion_outer, (180, 280), (1740, 610), 255, thickness=-1)
    title_motion_outer = cv2.GaussianBlur(
        title_motion_outer, (0, 0), sigmaX=55, sigmaY=32
    )
    title_motion_core = np.zeros((TARGET_H, TARGET_W), dtype=np.uint8)
    cv2.rectangle(title_motion_core, (225, 305), (1695, 590), 255, thickness=-1)
    title_alpha = np.maximum(title_alpha, title_motion_outer)
    title_alpha = np.maximum(title_alpha, title_motion_core)
...
def build_title_layers(brand: str, course: str,
    ...
    boxes, line_gap, total_h = _visual_text_metrics(draw, t_lines, t_font)
    visual_top = TITLE_CENTER_Y - total_h // 2
    visual_top = max(TITLE_SAFE_TOP, min(visual_top, TITLE_SAFE_BOTTOM - total_h))

    # Render a separate blurred shadow layer. This is much closer to the
    # source intro than the previous sharp 3 px / 5 px duplicate text.
    ...
    for line, bbox in zip(t_lines, boxes):
        w = bbox[2] - bbox[0]
        glyph_h = max(1, bbox[3] - bbox[1])
        x = TARGET_W // 2 - w // 2 - bbox[0]
        y = cursor - bbox[1]
        placements.append((x, y, line))
        cursor += glyph_h + line_gap

    sx, sy = TITLE_SHADOW_OFFSET
...
def build_pill_layers(brand: str, unit_no: str, chapter: str,
    ...
    border = BRANDS[brand].get("pill_border", INTRO_PILL_BORDER)
    ...
    while p_size > 24:
        ...
        if bbox[2] - bbox[0] + PILL_HORIZONTAL_PADDING <= 1700:
            break
        p_size -= 2
    p_font = make_font(p_size)
    bbox = draw.textbbox((0, 0), pill_line, font=p_font)
    ...
    pw = min(max(uw + PILL_HORIZONTAL_PADDING, PILL_MIN_W), 1700)
    ph = PILL_HEIGHT
    px = TARGET_W // 2 - pw // 2
    ...
    shadow_layer = Image.new("RGBA", (TARGET_W, TARGET_H), (0, 0, 0, 0))
    shadow_draw = ImageDraw.Draw(shadow_layer, "RGBA")
    psx, psy = PILL_SHADOW_OFFSET
    shadow_draw.rounded_rectangle(
        [px + psx, py + psy, px + pw + psx, py + ph + psy],
        radius=ph // 2,
        fill=(0, 0, 0, PILL_SHADOW_ALPHA),
    )
    shadow_layer = shadow_layer.filter(ImageFilter.GaussianBlur(PILL_SHADOW_BLUR))
    text_layer = Image.alpha_composite(text_layer, shadow_layer)
    draw = ImageDraw.Draw(text_layer, "RGBA")
...
def build_intro_logo_layer(brand: str) -> Image.Image:
    """Return the selected brand logo centred in the source intro logo area."""
    canvas = Image.new("RGBA", (TARGET_W, TARGET_H), (0, 0, 0, 0))
    logo_path = _logo_path(brand)
    if not logo_path:
        return canvas
    ...
    with Image.open(logo_path) as source:
        ...
        logo.thumbnail((INTRO_LOGO_MAX_W, INTRO_LOGO_MAX_H), Image.Resampling.LANCZOS)
        x = TARGET_W // 2 - logo.width // 2
        y = INTRO_LOGO_CENTER_Y - logo.height // 2
        canvas.alpha_composite(logo, (x, y))
    return canvas


def build_preview_logo_clean_patch(intro_path: Path) -> Image.Image:
    """Create a softly feathered clean patch for the static preview only.

    The rendered video uses FFmpeg's dynamic delogo filter, which follows the
    moving background more accurately. The preview uses an early clean frame.
    """
    clean_rgb = _read_intro_frame(intro_path, 0)
    if clean_rgb is None:
        return Image.new("RGBA", (TARGET_W, TARGET_H), (0, 0, 0, 0))

    x, y, w, h = INTRO_LOGO_DELOGO
    alpha = np.zeros((TARGET_H, TARGET_W), dtype=np.uint8)
    cv2.rectangle(alpha, (x, y), (x + w, y + h), 255, thickness=-1)
    alpha = cv2.GaussianBlur(alpha, (0, 0), sigmaX=20, sigmaY=14)
    rgba = np.dstack([clean_rgb, alpha]).astype(np.uint8)
    return Image.fromarray(rgba, "RGBA")
...
def make_preview(brand: str, course: str, unit_no: str, chapter: str) -> Optional[Image.Image]:
    ...
    if not intro:
        return None
    cap = cv2.VideoCapture(str(intro))
    cap.set(cv2.CAP_PROP_POS_MSEC, MASK_REFERENCE_TIME_MS)
    ok, frame = cap.read()
    cap.release()
    if not ok:
        return None
    ...
    logo_clean = build_preview_logo_clean_patch(intro)
    _, title_text = build_title_layers(brand, course, intro_path=intro)
    _, pill_text = build_pill_layers(brand, unit_no, chapter, intro_path=intro)
    logo_layer = build_intro_logo_layer(brand)

    composed = Image.alpha_composite(base, title_clean)
    composed = Image.alpha_composite(composed, title_text)
    composed = Image.alpha_composite(composed, pill_clean)
    composed = Image.alpha_composite(composed, pill_text)
    composed = Image.alpha_composite(composed, logo_clean)
    composed = Image.alpha_composite(composed, logo_layer)
...
def generate_intro_clip(out_path: Path, brand: str, course: str, unit_no: str,
    ...
    paths = {
        ...
        "brand_logo":  out_path.with_suffix(".brandlogo.png"),
    }
    try:
        title_clean, pill_clean = build_clean_patch_layers(intro)
        _, title_text = build_title_layers(brand, course, intro_path=intro)
        ...
        vf = (
            ...
            f"[5:v]format=rgba,fade=t=in:st={INTRO_LOGO_FADE_START}:d={INTRO_LOGO_FADE_DUR}:alpha=1[lg];"
            f"[base][tc]overlay=0:0:format=auto[s1];"
            f"[s1][tt]overlay=0:0:format=auto[s2];"
            f"[s2][pc]overlay=0:0:format=auto[s3];"
            f"[s3][ut]overlay=0:0:format=auto[s4];"
            f"[s4][lg]overlay=0:0:format=auto,format=yuv420p[v]"
        )
